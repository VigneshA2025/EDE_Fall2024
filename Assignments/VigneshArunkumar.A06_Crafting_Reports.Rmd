...
library(knitr)
EPAair <- data.frame(Item=c('Filename', 'Date', 'Source'),
Value=c('EPAair_O3_PM25_NC1819_Processed.csv','2018-2019', 'EPA Air Quality System (AQS)'))
kable(EPAair, align = c("r", "l"), caption = "Table: EPA Air Quality")
...
epa_data <- read.csv(
  here("Data","Processed","Processed_KEY","EPAair_O3_PM25_NC1819_Processed.csv"),
...
#Use the `kable` function to produce the 2 tables in markdown format
    ...
    kable(summary_PM25, col.names = c('County', "$\\mu g/m^3$"), caption = "Mean Particulates (2.5mm)", digits = 2)
    kable(summary_Ozone, col.names = c('County', "$\\mu g/m^3$"), caption = "Mean Ozone", digits = 2)
```
...
ozone2018 <- epa_data[epa_data$Year==2018,]

Plot1<- ggplot(ozone2018, aes(x = as.factor(Month)))+
  geom_boxplot(aes(y=Ozone))+
  labs(caption = "Figure 1: Monthly Ozone levels in 2018", x = "Month",y = 'Concentrations "$\\mu g/m^3$"', title = "Monthly Ozone Concentrations (2018)")
...
Plot2<- ggplot(ozone2019, aes(x = as.factor(Month)))+
  geom_boxplot(aes(y=Ozone))+
  labs(caption = "Figure 2: Monthly Ozone levels in 2019", x = "Month",y = 'Concentrations"$\\mu g/m^3$"', title = "Monthly Ozone Concentrations (2019)")
...
## Git Repository
https://github.com/VigneshA2025/EDE_Fall2024
